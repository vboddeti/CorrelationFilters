% build_zammcf_primal.m;
%
% * Citation: This function implements the zero-aliasing version of MMCF in the following publication.
...
function out = build_zammcf_primal
...
if ~args.cg
    [sol,obj] = primal_linear_newton(labels,lambda);
else
    [sol,obj] = primal_linear_cg(labels,lambda);
end;
...
function  [w,obj] = primal_linear_newton(labels,lambda)
% -------------------------------
% Train a linear ZAMMCF using Newton
% -------------------------------
global S;
global args;
global data_freq;
...
while 1
    ...
    step1 = h_prox(step(1:end-1),args);
    step(1:end-1) = step1;
...
function  [w, obj] = primal_linear_cg(labels,lambda)
% -----------------------------------------------------
% Train a linear ZAMMCF using nonlinear conjugate gradient
% -----------------------------------------------------
global args;
global data_freq;
[n,d] = size(data_freq);
...
% From the current solution w, do an exact line search in the direction d by
% 1-D Newton minimization
global T;
global args;
global data_freq;
...
val = sum(fusion_matrix_multiply(conj(w),grad,[1,dim],[dim,1]));
val = real(val);
